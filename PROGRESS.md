...
- **解决**：JWT 请求实时读取用户是否存在、状态和当前角色，删除成员时同步清理其账号分配；查询 token 同时保存 hash 与 AES-GCM 加密副本，旧库明文只有在能与 hash 对上时才迁移为加密副本，并统一用 hash 覆盖遗留列。新增撤权、停用、删除、降级和真实 legacy schema 迁移测试。
