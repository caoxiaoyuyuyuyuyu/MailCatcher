...
- **解决**：巡检口径明确为“IMAP 登录 + 打开 `INBOX`”，不发送探测邮件、不自动修改健康状态；默认并发 5、单账号超时 20 秒、单批最多 200 个。报告区分正常/异常/跳过，按账号权限过滤，错误统一替换明文与密文密码；非 IMAP 路径安全跳过。
- **避免**：健康检查必须写清检测层级（DNS/连接/认证/收件箱/真实投递）；批处理外部服务时始终限制并发、超时和批量大小，并把凭据脱敏纳入自动化测试。
