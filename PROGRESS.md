...
## 账号 token 授权复制（commit 4ea4061）

### 13. 可恢复凭据必须同时处理撤权与旧库明文迁移
- **问题**：为授权用户增加 token 复制后，若只验证 JWT 签名，成员停用、删除或降级后旧 JWT 仍保留原权限；旧库已有 `token_hash` 时也可能漏建加密副本并继续保留遗留明文。
- **解决**：JWT 请求实时读取用户是否存在、状态和当前角色，删除成员时同步清理其账号分配；查询 token 同时保存 hash 与 AES-GCM 加密副本，旧库明文只有在能与 hash 对上时才迁移为加密副本，并统一用 hash 覆盖遗留列。收回分配、停用或删除成员时只提示管理员按需轮换其可能已复制的账号 token，不自动轮换。新增撤权、停用、删除、降级和真实 legacy schema 迁移测试。
- **避免**：新增“查看/导出秘密”能力时，权限不能只信任长期 JWT 中的快照；凭据迁移必须覆盖“无 hash、有 hash、仅剩 hash”三种旧数据，并断言迁移后不再留明文。

## IMAP 批量巡检（commit 2b37846）

### 12. “能登录收件箱”与“能收到测试邮件”是两种健康口径
- **问题**：批量判断邮箱是否正常时，如果只写“收件正常”，容易让人误以为系统已验证真实邮件投递；直接并发所有账号还可能触发邮箱服务商限流，错误信息也可能夹带凭据。
- **解决**：巡检口径明确为“IMAP 登录 + 打开 `INBOX`”，不发送探测邮件、不自动修改健康状态；页面一次提交最多 200 个显式 ID 的异步批次并轮询进度，任务进入 BullMQ，由 Redis 在所有服务实例间限制默认 5 个连接和每分钟 30 次启动，并设置全局积压上限、同账号默认冷却 30 秒、单用户默认 10 分钟 200 次。报告区分正常/异常/跳过，按账号权限过滤，错误统一替换明文与密文密码；非 IMAP 路径安全跳过。
- **避免**：健康检查必须写清检测层级（DNS/连接/认证/收件箱/真实投递）；批处理外部服务时不能只限制单次函数调用的并发，还要考虑多用户、多请求和多实例叠加，必须同时设置跨实例全局并发和启动速率、积压上限、用户额度、账号冷却、批次上限和超时，并把凭据脱敏纳入自动化测试。

## Gazeta/Onet 网页邮箱适配（commit 3cad347）

### 10. Onet 登录成功不等于邮箱服务已启用
- **问题**：真实 Onet 账号可以完成 O!Konto 登录，但访问收件箱会被重定向到套餐选择页；自动点击免费套餐还会改变营销授权/账户状态。
