...
## IMAP 批量巡检（commit 5ee0639）

### 12. “能登录收件箱”与“能收到测试邮件”是两种健康口径
- **问题**：批量判断邮箱是否正常时，如果只写“收件正常”，容易让人误以为系统已验证真实邮件投递；直接并发所有账号还可能触发邮箱服务商限流，错误信息也可能夹带凭据。
- **解决**：巡检口径明确为“IMAP 登录 + 打开 `INBOX`”，不发送探测邮件、不自动修改健康状态；任务进入 BullMQ，由 Redis 在所有服务实例间限制默认 5 个连接，同账号默认冷却 30 秒、单用户默认 10 分钟 200 次；同步请求显式限制为 1–5 个 ID，页面再逐项处理最多 200 个可见账号。报告区分正常/异常/跳过，按账号权限过滤，错误统一替换明文与密文密码；非 IMAP 路径安全跳过。
- **避免**：健康检查必须写清检测层级（DNS/连接/认证/收件箱/真实投递）；批处理外部服务时不能只限制单次函数调用的并发，还要考虑多用户、多请求和多实例叠加，必须同时设置跨实例全局并发、用户额度、账号冷却、同步请求上限和超时，并把凭据脱敏纳入自动化测试。
