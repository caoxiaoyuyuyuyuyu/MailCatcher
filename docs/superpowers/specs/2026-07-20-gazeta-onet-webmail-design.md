...
为 Gazeta 增加受控浏览器网页适配器，为 Onet 增加官方 IMAP 路由并保留网页适配器作为显式回退；共享一组无状态 Playwright 工具。实测 Onet 官方 IMAP `imap.poczta.onet.pl:993` 可用，默认路径不依赖网页 reCAPTCHA；设置 `ONET_ACCESS_MODE=webmail` 才启用网页模式。Onet 网页模式若跳转到套餐选择页，适配器只识别并报告该激活门槛，不自动选择套餐、同意营销授权或付费。
...
- `server/src/services/onet.js`：处理 Onet 网页回退登录流程、套餐激活门槛/挑战识别并读取收件箱；默认取码走 `imap.poczta.onet.pl`。
...
4. `imap.js` 按实际收件邮箱域名路由：mail.com 系列走 `mailcom.js`，Gazeta 走 `gazeta.js`，Onet 默认走官方 IMAP（网页模式显式走 `onet.js`），其他域名保持原有 IMAP 行为。
5. 提供商适配器连接/登录并读取最近邮件，统一返回 `{ subject, from, body, links, date }`。
...
1. Gazeta 或 Onet 网页模式启动隔离的 Chromium 会话并建立匿名页面；Onet 默认不启动浏览器而连接官方 IMAP。
2. 网页模式填写邮箱地址和密码，完成允许范围内的 OAuth/重定向；遇到验证码、二步验证或 Onet 套餐页立即返回稳定错误。
...
- `@gazeta.pl` 能由网页适配器完成登录与最近邮件读取；`@onet.pl` 默认能由官方 IMAP 完成最近邮件读取，网页模式下套餐页、验证码/二步验证能返回稳定可定位错误。
