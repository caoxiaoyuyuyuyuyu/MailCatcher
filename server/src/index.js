...
app.get('/api/admin/stats', async (req, res) => {
  ...
  const servers = db.prepare('SELECT COUNT(*) as c FROM mail_servers').get().c;
  const logs = db.prepare('SELECT COUNT(*) as c FROM email_logs').get().c;
  const successLogs = db.prepare('SELECT COUNT(*) as c FROM email_logs WHERE success = 1').get().c;
