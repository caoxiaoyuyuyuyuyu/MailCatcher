...
cd server
npm install
npm test        # 运行单元、页面静态与集成测试（集成断言 65 条），均无需外网、确定性运行
```

覆盖：加密往返 / token hash + 加密副本、登录与双角色(admin/member)、自助注册、forward 转发取码、
**邮箱接码 + 用户 API Key**、成员权限隔离、管理员升降级(防自锁)、健康状态机、token 复制/轮换、删除外键、
...
# admin / owner / 被分配用户复制完整 token；旧账号需先轮换一次
...
6. 验证 token 复制：admin、账号归属人和被分配用户点击令牌列「复制」可得到完整 token；旧账号提示先轮换，轮换后可复制。
